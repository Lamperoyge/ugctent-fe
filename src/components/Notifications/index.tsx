...
import { MARK_NOTIFICATION_AS_VIEWED } from 'graphql/mutations/notification';
...
const Notifications = () => {
  ...
  const [markNotificationAsViewed] = useMutation(MARK_NOTIFICATION_AS_VIEWED, {
          ...
          <InfiniteScroll onLoadMore={handleFetchMore} hasMore={hasMore}>
            ...
            >
              {!notificationsData?.length && !loading ? (
                <span className='py-8 h-full w-full flex flex-col gap-4 justify-center items-center text-md text-gray-600'>
                  <span>{`:(`}</span>
                  ...
                  >
                    <li
                      ...
                      className={`p-4 hover:bg-gray-50 ${
                        !notification?.viewedAt ? 'bg-gray-100' : 'bg-white'
                      }`}
                    >
                      <div className='flex space-x-3'>
                        <ProfilePicture size='h-6 w-6' src={content?.avatar} />
