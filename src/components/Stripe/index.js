...
const CheckoutForm = ({application , handleClose }) => {
...
<h2 className='py-4 font-bold'>Pay: {application?.price || application.job.price} RON to {application?.creator?.firstName} {application?.creator?.lastName}</h2>
      <PaymentElement />
...
const StripeComponent = ({clientSecret, application, handleClose}) => {
  ...
  if(!stripePromise) return null;
     return <Modal opened={!!clientSecret} onClose={handleClose}>
       <LoadingOverlay overlayBlur={2} />
  <Elements stripe={stripePromise} options={options}> 
  <CheckoutForm application={application} handleClose={handleClose}/>
  </Elements>
    </Modal>
