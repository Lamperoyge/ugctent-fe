...
const ApplicationsList = ({ jobId }) => {
  ...
  const tabs = [
    { name: 'Pending', count: '2', status: JOB_APPLICATION_STATUS.IN_REVIEW },
    { name: 'Approved', count: '2', status: JOB_APPLICATION_STATUS.ACCEPTED },
    { name: 'Rejected', count: '2', status: JOB_APPLICATION_STATUS.REJECTED },
  ];
              ...
              {jobApplicationsLoading ? (
                ...
                jobApplications?.getJobApplications?.map((jobApplication) => (
                  <li key={jobApplication._id}>
                    <Link href={`/projects/${jobId}/applications/${jobApplication._id}`} className='group block'>
