  ...
  return (
    ...
    >
      <InfiniteScroll onLoadMore={fetchNextJobs} hasMore={hasMore}>
        {jobs?.map((job) => (
          <Link href={`/projects/${job?._id}`} className="relative cursor-pointer col-span-1 flex flex-col p-4 text-center bg-white border border-gray-300 rounded-lg shadow hover:shadow-xl transition-shadow">
